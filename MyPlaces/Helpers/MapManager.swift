...
//  MyPlaces
//
//  Created by Quasar on 13.04.2022.
//
...
class MapManager {
    ...
    // Cтроим маршрут от местоположения пользователя до заведения
     func getDirections(for mapView: MKMapView, previousLocation:(CLLocation) -> ()) {
        ...
        directions.calculate { (response, error) in
            ...
            guard let response = response else {
                self.showAlert(title: "Error", message: "Directions is not avalable")
                return
            }
